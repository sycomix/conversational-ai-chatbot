...
def audio_spectrogram(samples, window_size, stride, magnitude_squared):
    ...
    output_height = (
        0 if length_minus_windows < 0 else (
            1 + (length_minus_windows / stride))
    )
...
def mfcc_mel_filiterbank_init(sample_rate, input_length):
    ...
    for i in range(input_length):
        ...
        if (i < start_index) or (i > end_index):
            ...
            if channel >= 0:
                weights[i] = (
                    center_frequencies[int(channel) + 1] -
                    freq2mel(i * hz_per_sbin)
                ) / (
                    center_frequencies[int(channel) + 1]
                    - center_frequencies[int(channel)]
                )
            else:
...
def mfcc_mel_filiterbank_compute(
    mfcc_input, input_length, start_index, end_index, weights, band_mapper
):
    filterbank_channel_count_ = 40
    # Compute
...
def mfcc(spectrogram, sample_rate, dct_coefficient_count):
    ...
    for i in range(audio_channels):
        start_index, end_index, weights, band_mapper = mfcc_mel_filiterbank_init(
            sample_rate, spectrogram_channels
        )
        ...
        for j in range(spectrogram_samples):
            ...
            mel_filiter = mfcc_mel_filiterbank_compute(
                mfcc_input,
            ...
            mfcc_output[j, :] = dct_compute(
                mel_filiter, filterbank_channel_count, dct_coefficient_count, cosine
            )
