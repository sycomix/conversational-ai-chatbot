...
import deepspeech_openvino.utils.alphabet as alphabet_module
from deepspeech_openvino.utils.audio_features import (
    samples_to_melspectrum,
    melspectrum_to_mfcc,
)
from deepspeech_openvino.utils.ctcnumpy_beam_search_decoder import (
    CtcnumpyBeamSearchDecoder,
)
...
class DeepSpeechPipeline:
    def __init__(
        ...
        if alphabet is None:
            ...
            self.alphabet = alphabet_module.load_alphabet(
                alphabet
            )  # shall not include <blank> token
        else:
        ...
        self._load_net(
            model, model_bin_fname=model_bin, device=device, ie_extensions=ie_extensions
        )

        self.decoder = CtcnumpyBeamSearchDecoder(
            self.alphabet,
            self.beam_width,
    ...
    def _load_net(
        ...
        if model_bin_fname is None:
            model_bin_fname = (
                os.path.basename(model_xml_fname).rsplit(".", 1)[0] + ".bin"
            )
            model_bin_fname = os.path.join(
                os.path.dirname(model_xml_fname), model_bin_fname
            )
        ...
        for extension_device, extension_fname in ie_extensions:
            ...
            self.ie.add_extension(
                extension_path=extension_fname, device_name=extension_device
            )

        # Read IR
        self.net = self.ie.read_network(
            model=model_xml_fname, weights=model_bin_fname)
    ...
    def extract_mfcc(self, audio, sampling_rate):
        ...
        if (
            abs(sampling_rate - self.p["model_sampling_rate"])
            > self.p["model_sampling_rate"] * 0.1
    ...
    def extract_per_frame_probs(
        ...
        assert (
            self.exec_net is not None
        ), "Need to call mds.activate(device) method before mds.stt(...)"

        padding = np.zeros(
            (self.p["num_context_frames"] // 2, self.p["num_mfcc_dct_coefs"]),
            dtype=mfcc_features.dtype,
        )
        mfcc_features = np.concatenate(
            (padding, mfcc_features, padding)
        ...
        mfcc_features = np.lib.stride_tricks.as_strided(  # TODO: replace with conv1d
            mfcc_features,
            (num_strides, self.p["num_context_frames"],
             self.p["num_mfcc_dct_coefs"]),
            (
        ...
        for i in wrap_iterator(range(0, mfcc_features.shape[0], self.num_batch_frames)):
            ...
            probs.append(
                res["logits"].squeeze(1)
            )  # they are actually probabilities after softmax, not logits
            state_h = res[
